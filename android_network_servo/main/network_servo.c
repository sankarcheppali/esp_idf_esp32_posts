...
#define BROADCAST_IP_ADDR "192.168.1.255" // you might need to change this acroding to the subnet mask and gateway address
#define BROADCAST_PORT 3000
#define SERVC_NAME "network_servo"
...
void udp_boardcast(void *pvParam){
    char tag[] = "udp_boardcast_task";
    ESP_LOGI(tag,"udp_boardcast task started \n");
    int s;
    ...
    while(1){
        ...
        int opt_flag =1,sent_data;
        setsockopt(s,SOL_SOCKET,SO_BROADCAST,&opt_flag,sizeof(opt_flag));
        do{
            sent_data = sendto(s, SERVC_NAME,sizeof(SERVC_NAME),0,(struct sockaddr*)&cast_addr,sizeof(cast_addr));
            ESP_LOGI(tag, "boardcasted the service\n");
            vTaskDelay(10000 / portTICK_PERIOD_MS); // every 10 seconds
        }while(sent_data > 0);
        close(s);
        ESP_LOGI(tag, "... new sockeet will be opened in 10 seconds");
...
{	
    ...
    xTaskCreate(&udp_boardcast,"udp_boardcast",4048,NULL,5,NULL);
}
